    ...
    return(
        ...
        <div class="card mb-5 mb-lg-0">
          <div class="card-body">
            ...
            <ul class="fa-ul">
                {
                    props.data.features.map((plan)=>{
                        return <li className={plan.isEnabled ? '' : "text-muted"}>
                            <span class="fa-li"><i class={`fas ${plan.isEnabled ? 'fa-check' : 'fa-times'}`}></i></span>
                            {
            ...
            <div class="d-grid">
              <button class="btn btn-primary text-uppercase" onClick={()=>{props.handleSubscription(props.data.packageName)}}>Select a Plan</button>
            </div>
          </div>
        </div>
      </div>
